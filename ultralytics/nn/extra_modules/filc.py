...
class WindowFrequencyModulation_FMFFN(nn.Module):
    ...
    def forward(self, x):
        ...
        # 2. 动态计算 Padding，确保 H, W 能被 window_size 整除
        pad_h = (p1 - H % p1) % p1
        pad_w = (p2 - W % p2) % p2
        ...
        # 4. Rearrange: [B, C, H_pad, W_pad] -> [B, w1, w2, p1, p2, C]
        # 此时 H_pad = w1 * p1, W_pad = w2 * p2，一定能整除
        x_padded = rearrange(x_padded, 'b c (w1 p1) (w2 p2) -> b w1 w2 p1 p2 c', p1=p1, p2=p2)

        # 保存数据类型并转为 float32 进行 FFT
        x_dtype = x_padded.dtype
        x_padded = x_padded.to(torch.float32)

        # 5. 执行 2D 快速傅里叶变换 (RFFT)
        # 对最后两个空间维度 (p1, p2) 进行变换
        # 输出形状: [B, w1, w2, p1, p2//2+1, C] (实数输入变换后最后一维减半+1)
        x_freq = torch.fft.rfft2(x_padded, dim=(3, 4), norm='ortho')

        # 6. 应用频域权重
        # self.complex_weight 形状: [p1, p2//2+1, C, 1]
        # x_freq 形状: [B, w1, w2, p1, p2//2+1, C]
        # PyTorch 广播规则:
        #   weight 去掉最后的单维度 -> [p1, p2//2+1, C]
        #   x_freq 的后三个维度正好是 [p1, p2//2+1, C]
        #   前面的 [B, w1, w2] 会自动广播匹配
        weight = torch.view_as_complex(self.complex_weight).squeeze(-1)  # [p1, p2//2+1, C]

        x_freq = x_freq * weight

        # 7. 执行逆傅里叶变换 (IRFFT)
        # s 参数必须指定为原始的窗口大小 (p1, p2)
        x_padded = torch.fft.irfft2(x_freq, s=(p1, p2), dim=(3, 4), norm='ortho')

        # 8. 还原形状: [B, w1, w2, p1, p2, C] -> [B, C, H_pad, W_pad]
        x_padded = rearrange(x_padded, 'b w1 w2 p1 p2 c -> b c (w1 p1) (w2 p2)')

        # 9. 裁剪回原始尺寸 (如果之前做过 Padding)
        if pad_h > 0 or pad_w > 0:
